...
for iSub = 1 : numSubSq
    ...
    a = 1 / sqrt(3);
    gridSize = 50;
    x_local = linspace(0, a, gridSize);
    y_local = linspace(0, a, gridSize);
    for m = 1 : 9
        for i = 1 : gridSize
            for j = 1 : gridSize
                gamma_local(i, j) = atan(x_local(i)/a);
                theta_local(i, j) = pi / 2 - atan(y_local(j)/(a * sec(gamma_local(i, j))));
                omegaXplot(i, j) = cos(gamma_local(i, j)) * sin(theta_local(i, j));
                omegaYplot(i, j) = sin(gamma_local(i, j)) * sin(theta_local(i, j));
                omegaZplot(i, j) = cos(theta_local(i, j));
                solPlot(i, j) = ...
                    constants(1, m) + ...
                    constants(2, m) * omegaXplot(i, j) + ...
                    constants(3, m) * omegaYplot(i, j) + ...
                    constants(4, m) * omegaZplot(i, j) + ...
                    constants(5, m) * (omegaXplot(i, j) ^ 2 - omegaYplot(i, j) ^ 2) + ...
                    constants(6, m) * omegaZplot(i, j) ^ 2 + ...
                    constants(7, m) * omegaXplot(i, j) * omegaYplot(i, j) + ...
                    constants(8, m) * omegaXplot(i, j) * omegaZplot(i, j) + ...
                    constants(9, m) * omegaYplot(i, j) * omegaZplot(i, j);
            end
        end
        figure
        mesh(gamma_local, theta_local, solPlot, 'edgecolor', 'k', 'faceColor', 'interp');
        hold on
        colorbar
        ylabel(colorbar, sprintf('b_{%i}(\\gamma,\\theta)', m), 'FontSize', 18);
        xlabel('\gamma', 'FontSize', 18);
        ylabel('\theta', 'FontSize', 18);
        zlabel(sprintf('b_{%i}(\\gamma,\\theta)', m), 'FontSize', 18);
        omegaXtmp = cos(gammaStore(m)) * sin(thetaStore(m));
        omegaYtmp = sin(gammaStore(m)) * sin(thetaStore(m));
        omegaZtmp = cos(thetaStore(m));
        sol_tmp = ...
            constants(1, m) + ...
            constants(2, m) * omegaXtmp + ...
            constants(3, m) * omegaYtmp + ...
            constants(4, m) * omegaZtmp + ...
            constants(5, m) * (omegaXtmp ^ 2 - omegaYtmp ^ 2) + ...
            constants(6, m) * omegaZtmp ^ 2 + ...
            constants(7, m) * omegaXtmp * omegaYtmp + ...
            constants(8, m) * omegaXtmp * omegaZtmp + ...
            constants(9, m) * omegaYtmp * omegaZtmp;
        scatter3(gammaStore(m), thetaStore(m), sol_tmp+100.0, 300, 'filled', 'p', 'markerfacecolor', 'k');
    end
    %}
...
% Store QDFE-center quadrature data

...
for k = 1 : (iRef + 1) ^ 2
    ...
    for m = 1 : 4
        ...
        SA_ind(counter) = surfaceArea{k}(m);
        counter = counter + 1;
    end
end

fprintf('the max-to-min ratio for sub-sub-square areas: %f \n', SA_max/SA_min);
fprintf('test: %f \n', max(SA_ind)/min(SA_ind));
fprintf('the max-to-average ratio for sub-sub-square areas: %f \n', SA_max/mean(SA_ind));
fprintf('difference between total weights and pi/6: %E \n', abs(SA_sum - pi / 6));

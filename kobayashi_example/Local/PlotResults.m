...
%Reference = 0.00003382287819829766;
% LS solutions
LS = [8.8151441308E-05, ...
0.0000000000E+00, ...
0.0000000000E+00, ...
...
% LDFE-SQ-sa local solutions
LDFESQSALOCAL = [0.0000000000E+00, ...
5.0862222832E-05, ...
3.6295860644E-05, ...
3.1650507257E-05, ...
3.3106736493E-05, ...
...
% LDFE-SQ-sa relative error
LDFESQSAerrLOCAL = abs(LDFESQSALOCAL - Reference) / Reference;
...
1056, ...
3120];
for i = 1 : size(LDFESQmeshLOCAL, 2)
   LDFESQmeshLOCAL(i) = 1 / sqrt(LDFESQmeshLOCAL(i));     
end
...
1056, ...
3120];
for i = 1 : size(fourthMesh2, 2)
    fourthMesh2(i) = 1 / sqrt(fourthMesh2(i));
end
...
slope2 = 4;
intercept2 = log(LDFESQSAerrLOCAL(1));
figure
...
hold on
LDFESQSALOCALplot = loglog(LDFESQmeshLOCAL, LDFESQSAerrLOCAL, 'm-s');
set(LDFESQSALOCALplot, 'LineWidth', 1.25, 'MarkerSize', 10);
hold on
fourthOrder = loglog(fourthMesh, exp(slope .* log(fourthMesh / LDFESTmesh(1)) + intercept), 'r');
set(fourthOrder, 'LineWidth', 1.25);
fourthOrder2 = loglog(fourthMesh2, exp(slope2 .* log(fourthMesh2 / LDFESTmesh(1)) + intercept), 'r--');
set(fourthOrder2, 'LineWidth', 1.25);
grid on
set(gca,'FontSize', 14)
xlabel('Mesh Length', 'FontSize', 18)
ylabel('|Relative Error|', 'FontSize', 18)
legend('LS', 'GC', 'QR', 'LDFE-ST', 'LDFE-SQ', ...
     'QDFE-SQ', 'LDFE-SQ Local', '2nd-order', '4th-order', 'Location', 'Southeast')
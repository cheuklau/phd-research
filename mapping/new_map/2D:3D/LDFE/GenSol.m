function [sol, limits] = GenSol(quad, functionNumber, problem, factor_per)
...
elseif functionNumber == 4
    ...
    for i = 1 : numDirs
        ...
        if theta > min_theta - factor_per && theta < max_theta + factor_per && ...
           gamma > min_gamma - factor_per && gamma < max_gamma + factor_per
...
elseif functionNumber == 5
    ...
    for i = 1 : numDirs
        ...
        % Determine if current direction is within patch of interest
        if theta > pi / 8 && theta < pi / 4 && ...
                gamma > pi / 8 && gamma < 3 * pi / 8
           
            sol(i) = 1 + factor_per;
            
        elseif theta > pi / 4 && theta < 3 * pi / 8 && ...
                gamma > pi / 8 && gamma < 3 * pi / 8
...
for i = 1 : gridSize
    for j = 1 : gridSize
        ...
        elseif functionNumber == 2
            ...
            solRef(i, j) = 1 + factor_per * omegaXref(i, j) ^ 5 + omegaYref(i, j) ^ 5 + omegaZref(i, j) ^ 5;

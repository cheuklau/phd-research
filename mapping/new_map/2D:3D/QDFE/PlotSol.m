function PlotSol(quadTo, solTo, functionNumber, orderFrom, orderTo, problem, limits, useFixUp)
...
for i = 1 : gridSize
    ...
    for j = 1 : gridSize
        ...
        elseif functionNumber == 4
            ...
            if thetaTmp > min_theta && thetaTmp < max_theta && ...
                    gammaTmp > min_gamma && gammaTmp < max_gamma
...
for i = 1 : numDirsTo
    ...
    elseif functionNumber == 4        
        ...
        % Determine if current direction is within patch of interest
        if thetaPlot(i) > min_theta && thetaPlot(i) < max_theta && ...
                gammaPlot(i) > min_gamma && gammaPlot(i) < max_gamma
...
for i = 1 : 3
    ...
    for j = 1 : numSubSq
        ...
        for k = 1 : 9
            ...
            % Store error
            if functionNumber == 3 || functionNumber == 4
...
orderUseFrom = 3 ^ orderFrom - 1;
orderUseTo   = 3 ^ orderTo - 1;
numSubSqFrom = (orderUseFrom + 1) ^ 2 * 3;
numDirsFrom  = 9 * numSubSqFrom;

% Plot title
if useFixUp == 0
    
    title(sprintf('QDFE-SQ mapping without fixup: order %i to %i (%i to %i dirs)', ...
        orderUseFrom, orderUseTo, numDirsFrom, numDirsTo),'FontSize', 18);
    
else
    
    title(sprintf('QDFE-SQ mapping with fixup: order %i to %i (%i to %i dirs)', ...
        orderUseFrom, orderUseTo, numDirsFrom, numDirsTo),'FontSize', 18);
